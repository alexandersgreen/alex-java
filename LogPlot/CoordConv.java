...
public class CoordConv {
	...
	public double degminDeg(double degMin){
		...
		String deg = temp.substring(0,dotPos-two);
		String min = temp.substring(dotPos-two,temp.length());
		
		double Deg;
		
		try{
			Deg = (new Double(deg)).doubleValue();
		}catch(Exception e){
			Deg = 0.0;
		}
		...
		//System.out.println(" : "+deg+"-"+min+" : "+Deg+"-"+Min);
