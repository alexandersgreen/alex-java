...
public class LogMap extends JFrame {
	...
	public LogMap(String logFile[]){

		readings = new Vector(500);
		readingsConv = new Vector(500);
		joins = new Vector(5);
		CoordConv converter = new CoordConv();
		...
		canvas = new LogMapCanvas(readingsConv,joins);		
		
		this.setDefaultCloseOperation(EXIT_ON_CLOSE);
		this.getContentPane().add(canvas);
		this.setSize(new Dimension(421,685));
		String logFiles = "";
		for(int i=0;i<logFile.length;i++){
			logFiles = logFiles + " " + logFile[i];
		}
		this.setTitle("GPS Log Plot : "+logFiles);
	...
	public void loadFile(String file[]){
		try{	
			for(int i=0;i<file.length;i++){
				FileInputStream inS = new FileInputStream(file[i]);
	...
	public static void main(String[] args){
		if(args.length>0){
			LogMap main = new LogMap(args);
		}else{
			System.out.println("Usage: LogPlot inputFileNames");
			System.exit(-1);
		}
	}
	...
	public double getLat(String num, String dir){
		...
		if(dir.compareTo("S")==0){
			temp = new Double(temp.doubleValue() * -1.0);
		}
