...
public class Declared_BoolExpression extends asg01u.components.BoolExpression{
	...
	public Declared_BoolExpression(String font, int attrib, int size, Color col,String input,boolean root,boolean show,HashMap hm){
		super(font,attrib,size,col,root);
		variables = hm;
		expanded = show;
		value = input;
		System.out.println("Declared_BoolExpression(\""+input+"\") called");
		
		if(variables.containsKey(value)){
		BoolExpression temp = (BoolExpression) variables.get(value);
		try{
		var = (BoolExpression) temp.clone();
		}catch(Exception e){
	...
	public String outputString(){
		...
		output = "<Declared_BoolExpression>\n";
		...
		output = output + "\n    </blue>\n";
		output = output + "  </colour>\n";
		output = output + "</Declared_BoolExpression>\n";
		return output;	
	}
	...
	public Object clone() throws CloneNotSupportedException {
		return new Declared_BoolExpression(new String(fontName),fontAttributes,fontSize,new Color(colour.getRed(),colour.getGreen(),colour.getBlue()),new String(value),true,expanded,variables);
	}
}
