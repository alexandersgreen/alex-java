...
public class Or_BoolExpression extends asg01u.components.BoolExpression{

	asg01u.components.BoolExpression exp1;
	asg01u.components.BoolExpression exp2;
	JTextArea or;
	
	public Or_BoolExpression(String font, int attrib, int size, Color col,asg01u.components.BoolExpression in1,asg01u.components.BoolExpression in2,boolean root){
			super(font,attrib,size,col,root);
			exp1 = in1;
			exp2 = in2;
			System.out.println("Or_BoolExpression created");
			or = new JTextArea(""+'\u03B3');
			for(int i=0;i<4;i++){
			this.setTheFont(font,attrib,size,col,i);
			}
			or.setAlignmentX(Component.CENTER_ALIGNMENT);
			or.setAlignmentY(Component.CENTER_ALIGNMENT);
			or.setEditable(false);
			or.setMaximumSize(or.getPreferredSize());
		...
		public void setTheFont(String font, int attrib, int size, Color col, int what){
			...
			or.setFont(new Font(fontName,fontAttributes,fontSize));
			or.setForeground(colour);
			or.setMaximumSize(or.getPreferredSize());
			recreate();
			}
	...
	public String outputString(){
		...
		output = "<Or_BoolExpression>\n";
		output = output + "  <OrExpression1>\n";
			output = output + exp1.outputString();
		output = output + "  </OrExpression1>\n";
		output = output + "  <OrExpression2>\n";
		...
		output = output + "\n    </blue>\n";
		output = output + "  </colour>\n";
		output = output + "</Or_BoolExpression>\n";
		return output;	
	}
	...
	public Object clone() throws CloneNotSupportedException {
		return new Or_BoolExpression(new String(fontName),fontAttributes,fontSize,new Color(colour.getRed(),colour.getGreen(),colour.getBlue()),(BoolExpression) exp1.clone(),(BoolExpression) exp2.clone(),true);
	}			
}